...
class Object:
    def __init__(self, max_x: float, max_y: float, pos_x: float, pos_y: float, width: float, height: float = None) -> None:
        if height:
    ...
    def hitbox_circle(self, other):
        if self.type == HitboxType.CIRCLE:
            ...
            return not (   self.pos.x + self.radius < (other.pos.x - (other.width / 2))
    ...
    def hitbox_rectangle(self, other):
        if self.type == HitboxType.CIRCLE:
            ...
            return not (   self.pos.x + (self.width / 2) < (other.pos.x - (other.width / 2))
            ...
            if self.type == HitboxType.CIRCLE:
                if object.hitbox_circle(self):
                    if object.type == HitboxType.CIRCLE:
                        ...
                        max_length = dist(temp, object.pos) - (self.radius + object.radius)
